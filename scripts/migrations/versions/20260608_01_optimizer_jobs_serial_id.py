"""Phase 10.1: replace UUID primary key on optimizer_jobs with a sequential integer.

UUID→integer is not an in-place cast; the table is disposable telemetry (no user
data), so the migration drops and recreates it. Existing job history is lost —
acceptable and called out explicitly.
